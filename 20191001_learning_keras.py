...
# 定义优化器
reduce_lr=keras.callbacks.ReduceLROnPlateau(monitor='accuracy', factor=0.5, patience=3, verbose=0, mode='max', epsilon=0.005, cooldown=0, min_lr=0)
...
# 这次我们用fit()来训练网路
print('Training ------------')
model.fit(X_train, y_train, epochs=1000, batch_size=64,callbacks=[reduce_lr])
