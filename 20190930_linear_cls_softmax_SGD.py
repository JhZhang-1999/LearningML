...
def soft_loss(s,batch_index):
    loss=0
    grads=np.full((10000,10),0,dtype=float) # 初始化s的梯度矩阵
...
def linear_cls(x,x_val,W,b):
    ...
    for i in range(10000):
        ...
        if np.argmax(s_val[i])==y_val[i]:
            vacc+=1
    random_index=random.sample([i for i in range(10000)],256) # 抽出256个index
    loss,grads=soft_loss(s,random_index) # 分数矩阵设置完成，计算当前分数矩阵的loss和s带来的梯度
    for i in random_index:
        for j in range(10):
            gradb[0,j]+=grads[i,j]/10000
            for k in range(3072):
                gradW[j,k]+=grads[i,j]*x[i,k]/10000
    return loss,tacc,vacc,gradW,gradb

# data=open(r"D:\LIFE\WORK\4-University\Others\NeuralNetworks\LearningML\20190930data.txt",'w+')
print("epoch, loss, train acc, val acc")
# print("epoch, loss, train acc, val acc",file=data)
# data.close()
for t in range(max_epoch):
    #with open(r"D:\LIFE\WORK\4-University\Others\NeuralNetworks\LearningML\20190930data.txt",'w+') as data:
        #print(t,end="",file=data)
        print(t,end="")
        #print(",",end="",file=data)
        print(",",end="")
        loss,tacc,vacc,gradW,gradb=linear_cls(x,x_val,W,b)
        #print(loss,end="",file=data)
        print(loss,end="")
        #print(",",end="",file=data)
        print(",",end="")
        #print(tacc,end="",file=data)
        print(tacc,end="")
        #print(",",end="",file=data)
        print(",",end="")
        #print(vacc,file=data)
        print(vacc)
